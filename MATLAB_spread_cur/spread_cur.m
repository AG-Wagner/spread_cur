% spread_cur.m, version 08.07.2019
%
%  written bei Veit Wagner, Jacobs University Bremen gGmbH (https://www.jacobs-university.de/directory/vwagner)
%
%  The software below is a MATLAB-code (https://www.mathworks.com/products/matlab.html) and is free to use.
...
function [Ispread, jv, Voc, i0, Jv] = spread_cur(V, Imeas, A, C, Rsq)
    ...
    % --- input data check ---
    err = max([size(V,1),size(Imeas,1),size(A),size(C),size(Rsq)]~=1) | size(V,2) ~= size(Imeas,2) | size(V,2) < 2;
    if(err) 
        disp('spread_cur(): Error: wrong format of input data');
    else
        err = max(V(2:end) <= V(1:end-1));
        if(err) 
            disp('spread_cur(): Error: voltage values V not strictly increasing.'); 
        end
    end
    if(err) 
        Ispread=[]; jv=[]; Voc=[]; i0=[]; Jv=[]; 
        return; 
    end

    % --- calculation ---
    % -- find (first) Imeas=0 position -> Voc, i0 --
    i0 = find(Imeas(2:end).*Imeas(1:end-1) <= 0, 1);
    if( isempty(i0) || i0==size(Imeas,2)-1) 
        disp('spread_cur(): Error: can''t find zero crossing before last value of Imeas .'); Ispread=[]; jv=[]; Voc=[]; Jv=[]; 
        return; 
    end % error case 'not found'
    ...
    jvx = [0, jv(i0:-1:1)];
    Jv  = cumsum(.5*(jvx(2:end) + jvx(1:end-1)) .* (VV(2:end) - VV(1:end-1)));         % eqn 12
    Jv  = Jv(end:-1:1);
    VV  = [Voc, V(i0+1:end)];    % <- [Voc..Vend]
    jvx = [0, jv(i0+1:end)];
    Jv  = [Jv , cumsum(.5*(jvx(2:end) + jvx(1:end-1)) .* (VV(2:end) - VV(1:end-1)))];  % eqn 12
